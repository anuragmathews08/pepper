    ...
    <div className={classes.choose__block}>
      ...
      <div className={classes.choose__type}>
        <div className={classes.type__cardContainer}>
          <ChoiceCard img={bSelect ? CakeWhite : CakeDark} desc="Birthday Cake" title="Birthday" selected= {bSelect} click={() => BcardClicked()}/>
          <ChoiceCard img={aSelect ? RingWhite : RingDark} desc="Ring Card" title="Anniversary" selected = {aSelect} click={() => AcardClicked()}/>
        </div>
      </div>
      <div className={classes.choose__button}>
        <Button disable={isDisable}>Next</Button>
      </div>
    </div>
  );
}
